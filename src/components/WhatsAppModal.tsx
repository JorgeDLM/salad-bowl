...
export default function WhatsAppModal({ isOpen, onClose, selectedPackage }: WhatsAppModalProps) {
  ...
  const generateWhatsAppMessage = () => {
    ...
    if (selectedPackage) {
      message += `📦 *Paquete de interés:* ${selectedPackage.name}\n`;
      message += `💰 *Precio:* ${selectedPackage.price}\n`;
      message += `📋 *Descripción:* ${selectedPackage.description}\n\n`;
    } else {
      message += `🎯 *Interés:* ${formData.interest}\n\n`;
    }
    ...
    message += `Espero su respuesta para agendar una consulta. ¡Gracias!`;
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!selectedPackage && !formData.interest.trim()) {
      ...
      <div className="bg-gradient-to-br from-slate-900 to-purple-900 rounded-2xl p-8 max-w-md w-full border border-purple-500/30 shadow-2xl">
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-2xl font-bold text-white">Contactar por WhatsApp</h2>
          </div>
          <button
            onClick={onClose}
        ...
        {selectedPackage && (
          <div className="bg-purple-600/20 border border-purple-500/30 rounded-lg p-4 mb-6">
            <h3 className="font-bold text-purple-300 mb-1">{selectedPackage.name}</h3>
            <p className="text-purple-400 text-sm mb-2">{selectedPackage.price}</p>
            <p className="text-gray-300 text-xs">{selectedPackage.description}</p>
          </div>
        )}
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="Cuéntanos más sobre tu proyecto, presupuesto, timeline, etc."
            />
          </div>
